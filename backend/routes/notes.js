...
router.put("/updatenote/:id", fetchuser, async (req,res)=>{
    ...
    try {
       ...
       if(!note){return res.status(404).send("Not Found")} //Note is not there in the database
       if(note.user.toString() !== req.user.id){           //User is trying to access notes of other user    
...
router.delete("/deletenote/:id", fetchuser, async (req,res)=>{
  const {title, description, tag} = req.body;
  try {
     //Find the note to be deleted amd delete it
     let note = await Note.findById(req.params.id);
     if(!note){return res.status(404).send("Not Found")} //Note is not there in the database
